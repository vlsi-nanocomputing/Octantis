      ...
      if (opType == "null") {
        ...
        if(op == "add"){
          ...
          if (isMux) {
            ...
            printAddMux2to1(rowName, srcRow1, inputConnectionTypeSrc2, inputConnectionTypeSrc3, srcRow2, srcRow3,
                            sigDec, compInst);
  ...
  for(auto mapIT=((*compArray).limArray).begin(); mapIT != ((*compArray).limArray).end(); ++mapIT){
    if((mapIT->second).rowType == "load"){
      ...
      for(auto it=((*compArray).limArray).begin(); it != ((*compArray).limArray).end(); ++it){
        for(auto listIT = ((it->second).inputConnections).begin(); listIT != ((it->second).inputConnections).end(); ++listIT){
...
void PrintVhdlFile::printProcesses(raw_fd_ostream *const &compFd, raw_fd_ostream *const &sigFd){
    ...
    if(FSMIT->first != 0){
      ...
      for (FSM_ListIT=(FSMIT->second).begin(); FSM_ListIT!=(FSMIT->second).end(); ++FSM_ListIT) {       
        ...
        if(compArray->isLiMRowOfThisType(*FSM_ListIT, shiftType)){
          if(std::find(firstEnteredShift.begin(), firstEnteredShift.end(), *FSM_ListIT) == firstEnteredShift.end()){
            *compFd << "\t\t\t\t\tMem(" << std::stoi(rowAddr.str(), 0, 16)/4 << ") <= ROW_" << ((limArrayIT->second).inputConnections).front() << "_OUT;\n";
            firstEnteredShift.push_back(*FSM_ListIT);
          }else{
            *compFd << "\t\t\t\t\tMem(" << std::stoi(rowAddr.str(), 0, 16)/4 << ") <= '0' & ROW_" << *FSM_ListIT << "_MEM_RD(" << par << " - 1 downto 1);\n";
          }
          
        }else{
          if(((limArrayIT->second).inputConnectionsType).front() == "OLiM" || ((limArrayIT->second).inputConnectionsType).front() == "none"){
            *compFd << "\t\t\t\t\tMem(" << std::stoi(rowAddr.str(), 0, 16)/4 << ") <= ROW_" << ((limArrayIT->second).inputConnections).front() << "_OUT;\n";
          }else{
            *compFd << "\t\t\t\t\tMem(" << std::stoi(rowAddr.str(), 0, 16)/4 << ") <= ROW_" << ((limArrayIT->second).inputConnections).front() << "_MEM_RD;\n";
          }
  ...
  *compFd << "\tmux_proc: process (CLK, RST)\n" 
  ...
  for(auto listIT = (compFSM->selectionSignalsList).begin(); listIT != (compFSM->selectionSignalsList).end(); ++listIT){
...
void PrintVhdlFile::printTestbench(){
  ...
  *tbFile << "\tcomp: EntityName generic map (word_size => " << par << ", entries =>"  << entries << ")\n";
  ...
  std::string cntProc = "\tcnt_proc: process(rst_i, clk_i) \n"
                        ...
                        "\t\t\tend if;\n"
                        "\t\t\t\n"
                        "\t\t\taddress_i <= std_logic_vector(to_unsigned(cnt, " + std::to_string(par) + "));\n"
                        "\tend process;\n\n";
