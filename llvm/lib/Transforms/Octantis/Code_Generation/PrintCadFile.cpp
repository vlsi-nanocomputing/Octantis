...
void PrintCadFile::printAddLimcad() {
    ...
    if (ErrorInfolimcad) {
        errs() << "Error opening file: " << limcadCellName << "\n";
        llvm_unreachable(
                "Error in PrintVhdlFile: unable to create the Memory cell .limcad file associated to an ADD row.");
    }
...
void PrintCadFile::printAddMux2to1Limcad() {
    ...
    if (ErrorInfolimcad) {
        errs() << "Error opening file: " << limcadCellName << "\n";
        llvm_unreachable(
                "Error in PrintVhdlFile: unable to create the Memory cell .limcad file associated to an AddMux2to1 row.");
    }
...
void PrintCadFile::printLimcadCsvFile() {
    ...
    for (limArrayIT = ((*compArray).limArray).begin(); limArrayIT != ((*compArray).limArray).end(); ++limArrayIT) {
        ...
        std::transform((limArrayIT->second).rowType.begin(), (limArrayIT->second).rowType.end(),
                       (limArrayIT->second).rowType.begin(), ::tolower);
