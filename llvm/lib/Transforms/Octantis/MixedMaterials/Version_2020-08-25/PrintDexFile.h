...
//    int getIndex(std::string operation);
    ...
    void identifyLIMRowAndPrint(int *currentRow, std::map<int *, LiMArray::LiMRow>::iterator * mapIT);

    /// It is invoked when a normal memory row is declared
    void addDataRow(int * currentRow, int ** nameRow);

    /// It prints the correct code for any BITWISE lim cell
    void printBITWISE(std::string bitwiseOp, int * currentRow, int ** nameRow, int ** nameSrc);

    /// It prints the correct code for an SUM lim cell
    void printADD(int * currentRow, int **nameRow, int **nameSrc);
    ...
    void printBITWISEMux2to1(std::string bitwiseOp, int * currentRow, int **nameRow, int **nameSrc1, int **nameSrc2);

    /// It prints the correct code for an SUM lim cell with a 2to1 mux in input
    /// so that 2 input bits for the logic are available.
    void printADDMux2to1(int * currentRow, int **nameRow, int **nameSrc1, int **nameSrc2);
...
//    void printSUB(int * currentRow, int **nameRow, int **nameSrc);
    ...
    void getNameAndIndexOfSourceRow(int **sourceRow, std::string * sourceCellName, int * sourceCellRow);

    /// It inserts a new element inside the the namesMap
    void insertNamesMap(int **rowName, std::string cellName, int * cellRow);
...
private:
    ...
    std::map<int *, LiMArray::LiMRow>::iterator limArrayIT;
    ...
    std::map <int *, nameAndIndex> namesMap;

    //Iterator over the namesMap
    //std::map <int *, std::string>::iterator namesMapIT;
    std::map <int *, nameAndIndex>::iterator namesMapIT;
