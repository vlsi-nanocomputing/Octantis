...
int AddressList::extractIteratorAdvancement(const AccessPattern::arrayAccessPattern &ap, const int &i, const int &j,
                                            const int &k, const int &l, const int &arrayCols, const int &setCols) {
...
void AddressList::shapeAddressList(PointerInfoTable::pointerInfoStruct pis) {
    ...
    if (ap == AccessPattern::rowMajor || ap == AccessPattern::colMajor) {
        ...
        if (pis.pointerAccessPattern.getInitZeros() == 2) {

            for (int i = pis.offsetY; i < pis.stopX; i = i + pis.spacingX) {

                for (int j = pis.offsetX; j < pis.stopY; j = j + pis.spacingY) {

                    for (int k = 0; k <= i; k = k + pis.spacingInSubsetX) {

                        for (int l = 0; l <= j; l = l + pis.spacingInSubsetY) {
                            advance(addrListIT, pis.arrayCols * k + l);
...
std::list<int *> AddressList::getSubset(const int &subsetNumber, const int &nSubsetElements, const int &skipElements) {
    ...
    for (int j = 0; j < addrList.size() && !foundSubset; j += nSubsetElements) {
        ...
        if (subsetNumber == 1) {
            advance(addrListIT, 1);
            currSubsetNumber = subsetNumber;
        } else {
            advance(addrListIT, skipElements);
            currSubsetNumber = subsetNumber;
        };
