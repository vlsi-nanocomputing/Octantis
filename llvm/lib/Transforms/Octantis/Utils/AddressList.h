...
/// Class useful to store and handle a list of addresses of LiM rows on which an operation must be executed
    ...
    public:
        ...
        std::list<int *> getSubset(const int &subsetNumber, const int &nSubsetElements, const int &skipElements);
        ...
        int extractIteratorAdvancement(const AccessPattern::arrayAccessPattern &ap, const int &i, const int &j,
