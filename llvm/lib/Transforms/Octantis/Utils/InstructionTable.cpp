...
void InstructionTable::insertInstructionWithSpecs(int ti, int di, std::string operation, std::list<std::string> specs,
                                                  int *destReg, operandType destRegType,
                                                  int *destRegPrt, int *srcReg1, operandType srcReg1Type,
                                                  int *srcReg1Ptr, int *srcReg2, operandType srcReg2Type,
                                                  int *srcReg2Ptr, bool isInLoopBody) {
...
int InstructionTable::insertNewInstructionDestReg(std::string operation, int *newDestReg, int *oldDestReg) {
    ...
    instructionMap.insert(instructionMapIT,
                          std::pair<int *, InstructionTable::instructionData>(newDestReg, dataStruct));
...
void InstructionTable::printInstructionMap() {
    ...
    errs()
            << "|-------------------------------------------------------------------------------------------------------------------------------|\n";
    errs()
            << "|OP             |Dest Reg       |Dest Reg T     |Src Reg 1      |Src Reg 1 T    |Src Reg 2      |Src Reg 2 T    |Others         |\n";
    errs()
            << "|-------------------------------------------------------------------------------------------------------------------------------|\n";
    ...
    for (instructionMapIT = instructionMap.begin(); instructionMapIT != instructionMap.end(); ++instructionMapIT) {
        ...
        errs() << "|" << (instructionMapIT->second).operation;
        for (int i = (instructionMapIT->second).operation.length(); i < 15; ++i) { errs() << " "; }
        errs() << "|";
        errs() << ssDest.str();
        for (int i = ssDest.str().length(); i < 15; ++i) { errs() << " "; }
        errs() << "|";
        errs() << destT;
        for (int i = destT.length(); i < 15; ++i) { errs() << " "; }
        errs() << "|";
        errs() << ssSrc1.str();
        for (int i = ssSrc1.str().length(); i < 15; ++i) { errs() << " "; }
        errs() << "|";
        errs() << src1T;
        for (int i = src1T.length(); i < 15; ++i) { errs() << " "; }
        errs() << "|";
        errs() << ssSrc2.str();
        for (int i = ssSrc2.str().length(); i < 15; ++i) { errs() << " "; }
        errs() << "|";
        errs() << src2T;
        for (int i = src2T.length(); i < 15; ++i) { errs() << " "; }
        errs() << "|\n";
        errs() << (instructionMapIT->second).destRegPrt << " " << (instructionMapIT->second).srcReg1Ptr << " "
               << (instructionMapIT->second).srcReg2Ptr << "\n";
        errs()
                << "|-------------------------------------------------------------------------------------------------------------------------------|\n";
