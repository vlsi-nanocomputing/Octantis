...
void LiMArray::addNewLiMRow(int *const &rowName, std::string &rowType, std::list<std::string> &addLogic, int &rowLength,
                            int *const &src, std::string const &conType) {
...
bool
LiMArray::changeLiMRowType(int *const &rowName, std::string &newRowType, std::list<std::string> &additionalOperators) {
    ...
    if (!checkIfRowIsLiM(limArrayIntIT)) {
        ...
        if (!(limArrayIntIT->second.additionalLogic).empty()) {
            ...
            specs = find((limArrayIntIT->second.additionalLogic).begin(), (limArrayIntIT->second.additionalLogic).end(),
                         "input_line");
...
void LiMArray::printLiMArray() {
    ...
    if (limArray.size() == 0) {
        ...
        for (limArrayDebugIT = limArray.begin(); limArrayDebugIT != limArray.end(); ++limArrayDebugIT) {
            ...
            errs() << "Row " << rowCount << ", name " << limArrayDebugIT->first << ": Type "
                   << (limArrayDebugIT->second).rowType
            ...
            errs() << "\ninputs\t";
            for (inList = ((limArrayDebugIT->second).inputConnections).begin();
                 inList != ((limArrayDebugIT->second).inputConnections).end(); ++inList) {
                errs() << *inList;
            ...
            errs() << "\ntype  \t";
            for (inTypeList = ((limArrayDebugIT->second).inputConnectionsType).begin();
                 inTypeList != ((limArrayDebugIT->second).inputConnectionsType).end(); ++inTypeList) {
                errs() << *inTypeList;
...
void LiMArray::printLiMArrayToFile(const int &par) {

    std::ofstream outputFile;

    outputFile.open("LiMArray.txt");
    ...
    //Check if the map is empty:
    if (limArray.size() == 0) {
        outputFile << "\tERROR! The input map is empty!";
    } else {
        for (limArrayDebugIT = limArray.begin(); limArrayDebugIT != limArray.end(); ++limArrayDebugIT) {
            ...
            /*outputFile << "Row " << rowCount << ", name " << limArrayDebugIT->first << ": Type "
                   << (limArrayDebugIT->second).rowType
                   << ", length " << (limArrayDebugIT->second).rowLength << " additional integrated logic ";*/
        ...
        outputFile << "Total number of memory rows: " << limArray.size() << "\n";
        outputFile << "Total Number of bits: " << limArray.size()*par << "\n\n";
        outputFile << "[Row Type -> Count]\n";
        outputFile << "LOAD -> " << loadN << "\n";
        outputFile << "ADD -> " << addN << "\n";
        outputFile << "SHR -> " << sdivN << "\n";
        outputFile << "XOR -> " << sXor << "\n";
        outputFile << "Number of rows with muxes: " << nMux << "\n";
