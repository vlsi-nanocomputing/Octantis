...
LiMBinder::LiMBinder(InstructionTable &instructionMap, CollectInfo &infoCollection, bool debugMode, const int &par, const std::string &optTarget)
        : zeroAddr(0),
          InfoCollection(
                  infoCollection),
          IM(instructionMap),
    ...
    for (auto instructionOrderVectorIT = (IM.instructionOrderVector).begin();
         instructionOrderVectorIT != (IM.instructionOrderVector).end(); ++instructionOrderVectorIT) {
    ...
    MemArray.printLiMArrayToFile(par);
    FSMLim.transform();
    FSMLim.printFSMToFile();
    //}
...
void LiMBinder::handleAccumulationBindingProcess() {
    ...
    //Source reg 2 is always the set of elements to be accumulated
    if (srcReg1Type == InstructionTable::singleVariable || srcReg1Type == InstructionTable::fakeArray) {
        //Src reg 1 is a single variable
    ...
    if (debugMode) {
        errs() << "\tAddress List for src1 (dest):\n";
        addrListSrc1.printAddressList();
    }



    //Source reg 2 is always the destination where the accumulation result is put
    if (srcReg2Type == InstructionTable::array) {
        ...
        addrListSrc2.setAddressList(arrayNamesMapIT->second);

    }

    if (debugMode) {
        errs() << "\tAddress List for src2 (acc set):\n";
    ...
    if (!InfoCollection.LIT.getLoopInfo(srcReg1PointerInfo.firstIdxInfo.iterators.front()).isFinalValueConstant) {

        for (int i = 0; i < srcReg2PointerInfo.numberOfSubsetsElements; i++) {

            if (i / srcReg2PointerInfo.arrayCols == 0) {
                tmpAccList = addrListSrc1.getSubset(i, i + 1, skipElements);
                skipElements = skipElements + i + 1;

            } else {
                tmpAccList = addrListSrc1.getSubset(i, (i / srcReg2PointerInfo.arrayCols + 1) *
                                                       (i % srcReg2PointerInfo.arrayCols + 1), skipElements);
                skipElements += (i / srcReg2PointerInfo.arrayCols + 1) *
                                (i % srcReg2PointerInfo.arrayCols + 1);
            }

            tmpAccList.push_back(addrListSrc1(i));


            performAccBinding(tmpAccList, operation, specifications, srcReg2, destReg, ti);
        ...
        for (int i = 0; i < srcReg2PointerInfo.numberOfSubsets; i++) {
...
void LiMBinder::handleFPOperationBindingProcess() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (debugMode) {
            errs() << "\n\n\t\t\tMapping a " << operation << " operation with src1: " << src1 << ", src2: " << src2
                   << ", dest: " << dest << "\n\n";
        }
    ...
    if (debugMode) {
        errs() << "\t\tSource reg 1 is of type: " << srcReg1Type << ", source reg 2 is of type: " << srcReg2Type
               << " and dest reg is of type: " << destRegType << "\n"
               << "\t\t\tsrc1 : " << src1 << "\n"
               << "\t\t\tsrc2 : " << src2 << "\n"
               << "\t\t\tdest : " << dest << "\n";
    }
...
void LiMBinder::performAccBinding(std::list<int *> accumulationList, std::string operation,
    ...
    int time = 0, timeLimit;

    if(optTarget == "Accumulation"){
        timeLimit = steps;
    }else{
        timeLimit = 0;
    }
    ...
    while (isReduceable && time < timeLimit) {
...
void LiMBinder::performBinding() {
    ...
    if (lastDestMapIT1 != lastDestMap.end() && lastDestMapIT2 != lastDestMap.end() &&
        lastDestMapIT1->second == lastDestMapIT2->second) {
        ...
        if (op == operation) {
            ...
            if (debugMode) {
                errs() << "\t\t\t\tThe two source operands already generated " << lastDestMapIT1->second << " with a "
                       << operation << ", the result will be reused\n";
            }
        ...
        if (!(operation == "shl" || operation == "shr" || operation == "ashr")) {
            ...
            if (MemArray.changeLiMRowType(src2, operation, specifications)) {
            ...
            } else if (MemArray.isLiMRowOfThisType(src2, operation) &&
                       (((MemArray.findRow(src2))->second).inputConnections).size() < 3) {
                ...
                if (debugMode) {
                    errs() << "\t\t\t\t" << src2 << " is already of type " << operation
                           << ", a new input connection is inserted, " << src1 << "\n";
                }
            ...
            } else if (MemArray.isLiMRowOfThisType(src1, operation) &&
                       (((MemArray.findRow(src1))->second).inputConnections).size() < 3) {
                ...
                if (debugMode) {
                    errs() << "\t\t\t\t" << src1 << " is already of type " << operation
                           << ", a new input connection is inserted, " << src2 << "\n";
                }
                ...
                if (debugMode) {
                    errs() << "\t\t\t\tCopied " << src1 << " into copyRow " << copyRow << " ( " << tCopy
                           << ") with input " << src2 << "\n";
                }
