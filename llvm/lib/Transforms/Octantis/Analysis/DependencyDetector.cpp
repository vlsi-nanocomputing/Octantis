...
void DependencyDetector::handleInstructionInserting(const Instruction &I) {
    ...
    if (operation != "null" && operation != "load") {
        ...
        if ((int *) (I.getOperand(0)) == destReg || (int *) (I.getOperand(1)) == destReg) {
            ...
            if ((int *) (I.getOperand(0)) == destReg) {
                ...
                if (srcReg1 == nullptr) {
                    ...
                    srcReg1Type = destRegType;
                    tmpPtrSrc1 = tmpPtrDest;
                    setRegType(isSrc2Array, srcReg2, "srcReg2Type");
                } else if (srcReg2 == nullptr) {
            ...
            if (tmpPtrSrc1 != nullptr || tmpPtrSrc2 != nullptr) {
                ...
                if ((srcReg1Type == InstructionTable::array || srcReg1Type == InstructionTable::fakeArray) &&
                    (srcReg2Type == InstructionTable::singleVariable || srcReg2Type == InstructionTable::constant)) {

                    tmpPtrDest = tmpPtrSrc1;

                } else if ((srcReg1Type == InstructionTable::singleVariable ||
                            srcReg1Type == InstructionTable::constant) &&
                           (srcReg2Type == InstructionTable::array || srcReg2Type == InstructionTable::fakeArray)) {

                    tmpPtrDest = tmpPtrSrc2;

                } else if ((srcReg1Type == InstructionTable::array || srcReg1Type == InstructionTable::fakeArray) &&
                           (srcReg2Type == InstructionTable::array || srcReg2Type == InstructionTable::fakeArray)) {
                    ...
                    tmpPtrDest = (pisSrc2.numberOfSubsets > pisSrc1.numberOfSubsets) ? tmpPtrSrc1 : tmpPtrSrc2;
                }
        ...
        IM.insertInstruction(0, 0, operation, destReg, destRegType, tmpPtrDest, srcReg1, srcReg1Type, tmpPtrSrc1,
                             srcReg2, srcReg2Type, tmpPtrSrc2, isInLoopBody);
...
void DependencyDetector::handleStoreInstruction(const Instruction &I) {
    ...
    if (debugMode) {
        errs() << "srcReg1: " << srcReg1 << ", loadCount: " << loadCount << ", allocatedReg: " << allocatedReg
               << ", isAllocatedRegArray: " << isAllocatedRegArray << "\n\n";
    }
...
void DependencyDetector::detectDependencies(std::list<BasicBlock *> basicBlocksToSchedule) {
                    ...
                    */
                    ...
                    if (I.getOpcode() == Instruction::Add) {
                        ...
                        if (ConstantInt *CI = dyn_cast<ConstantInt>(I.getOperand(1))) {
                            ...
                            if(isNegated && tmpLogicOperatorDestReg == (int *) (I.getOperand(1))){
                    ...
                    InstructionTable::operandType oT = (infoCollection.isArray(allocatedReg)) ? InstructionTable::array
                                                                                              : InstructionTable::singleVariable;
                    ...
                    //hence, info about lastUsedRegMap are updated here, also checking if the destReg is an array,
    ...
    if (debugMode) {
        ...
        errs() << "\n\n\n\n\n\n\n\n";
        infoCollection.PIT.printPointerInfoTable();
        errs() << "\n\n\n\n\n\n\n\n";
    }
